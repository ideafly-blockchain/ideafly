...
// Package npos implements the proof-of-stake-authority consensus engine.
package npos
...
type bannedDirection uint

const (
	DirectionFrom bannedDirection = iota
	DirectionTo
	DirectionBoth
)
...
var (
	epochLength = uint64(200) // Default number of blocks after which to checkpoint and ranking the current votes

	extraVanity = params.NposExtraVanity // Fixed number of extra-data prefix bytes reserved for validator vanity
	extraSeal   = params.NposExtraSeal   // Fixed number of extra-data suffix bytes reserved for validator seal
...
// Npos is the proof-of-stake-authority consensus engine proposed to support the
// NDDN blockchain.
type Npos struct {
	chainConfig *params.ChainConfig // ChainConfig to execute evm
	config      *params.NposConfig  // Consensus engine configuration parameters
...
// New creates a NPoS proof-of-stake-authority consensus engine with the initial
// validators set to the ones provided by the user.
func New(chainConfig *params.ChainConfig, db ethdb.Database) *Npos {
	// Set any missing consensus parameters to their defaults
	conf := *chainConfig.Npos
	if conf.Epoch == 0 {
...
func (c *Npos) SetChain(chain consensus.ChainHeaderReader) {
	c.chain = chain
}

// SetStateFn sets the function to get state.
func (c *Npos) SetStateFn(fn StateFn) {
	c.stateFn = fn
}

// Author implements consensus.Engine, returning the Ethereum address recovered
// from the signature in the header's extra-data section.
func (c *Npos) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
	// return ecrecover(header, c.signatures)
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (c *Npos) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
...
func (c *Npos) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
func (c *Npos) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Npos) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Npos) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (c *Npos) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (c *Npos) VerifySeal(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// from.
func (c *Npos) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Npos) PreHandle(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
...
func (c *Npos) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	if number%c.config.Epoch == 0 {
		...
		newSortedValidators, err := c.getTopValidators(ctx)
		if err != nil {
			return err
		}
...
func (c *Npos) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs *[]*types.Transaction, uncles []*types.Header, receipts *[]*types.Receipt, systemTxs []*types.Transaction) error {
	...
	if header.Difficulty.Cmp(diffInTurn) != 0 {
		if err := c.tryPunishValidator(ctx, chain); err != nil {
			return err
		}
	}
	...
	if len(*txs) > 0 {
		if err := c.trySendBlockReward(ctx); err != nil {
			return err
		}
	}
	...
	if header.Number.Uint64()%c.config.Epoch == 0 {
		newValidators, err := c.syncWithSysContractAtEpoch(ctx, chain)
		if err != nil {
			return err
		}
	...
	proposalCount, err := c.getPassedProposalCount(ctx)
	if err != nil {
		return err
	}
	...
	for i := uint32(0); i < proposalCount; i++ {
		prop, err := c.getPassedProposalByIndex(ctx, i)
		if err != nil {
			return err
		}
		// execute the system governance Proposal
		tx := systemTxs[int(i)]
		receipt, err := c.replayProposal(ctx, prop, len(*txs), tx)
		if err != nil {
			return err
		}
		*txs = append(*txs, tx)
	...
	// Finish all proposal
	for i := uint32(0); i < proposalCount; i++ {
		err = c.finishProposalById(ctx, pIds[i])
		if err != nil {
			return err
		}
	}
...
func (c *Npos) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (b *types.Block, rs []*types.Receipt, err error) {
	...
	// do epoch thing at the end, because it will update active validators
	if header.Number.Uint64()%c.config.Epoch == 0 {
		if _, err := c.syncWithSysContractAtEpoch(ctx, chain); err != nil {
			panic(err)
		}
	}
	...
	if c.signTxFn != nil {
		...
		for i := uint32(0); i < proposalCount; i++ {
			...
			// execute the system governance Proposal
			tx, receipt, err := c.executeProposal(ctx, prop, len(txs))
			if err != nil {
				return nil, nil, err
			}
			txs = append(txs, tx)
...
// with.
func (c *Npos) Authorize(validator common.Address, signFn ValidatorFn, signTxFn SignTxFn) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *Npos) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
	...
	// Sign all the things!
	sighash, err := signFn(accounts.Account{Address: val}, accounts.MimetypeNpos, NposRLP(header))
	if err != nil {
		return err
	}
...
func (c *Npos) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
func (c *Npos) SealHash(header *types.Header) common.Hash {
	return SealHash(header)
}

// Close implements consensus.Engine. It's a noop for NPoS as there are no background threads.
func (c *Npos) Close() error {
	return nil
}

// APIs implements consensus.Engine, returning the user facing RPC API to allow
// controlling the validator voting.
func (c *Npos) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "npos",
		Version:   "1.0",
		Service:   &API{chain: chain, npos: c},
		Public:    false,
	}}
}
...
// NposRLP returns the rlp bytes which needs to be signed for the proof-of-stake-authority
...
func NposRLP(header *types.Header) []byte {
	b := new(bytes.Buffer)
