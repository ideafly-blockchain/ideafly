...
async function step1(validator) {
    ...
    try {
        const st1 = await vote1.state();
            console.log("✅ SUCCESS step1: before staking vote state",st1);
    } catch (e) {
        console.error("❌ FAILED step1: before staking vote state",st1);
        throw e;
    }
...
async function step2() {
    ...
    try {
        ...
        receipt = await tx.wait();
        expect(receipt.status).equal(1);
        console.log("✅ SUCCESS step2: vote2.deposit 10.0 eth");
    } catch (e) {
        console.error("❌ FAILED step2: vote2.deposit 10.0 eth");
        throw e;
    }
}

async function step3() {
    const vote3 = await ethers.getContractAt(VotePool_ABI, vp_pos2, manager);

    try {
        const tx = await vote3.exit();
        receipt = await tx.wait();
        expect(receipt.status).equal(1);
